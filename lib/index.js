...
function f(e, t = Infinity) {
	let { requestIdleCallback: n } = window;
	return n ? new Promise((e) => n.call(window, () => e(), { timeout: t })) : u(Math.min(e, t));
}
function p(e) {
...
function m(e, t) {
	try {
		let n = e();
		p(n) ? n.then((e) => t(!0, e), (e) => t(!1, e)) : t(!0, n);
	} catch (e) {
		t(!1, e);
	}
}
async function h(e, t, n = 16) {
	let r = Array(e.length), i = Date.now();
...
var ae = [4283543511, 3981806797], oe = [3301882366, 444984403];
function se(e) {
	let t = [0, e[0] >>> 1];
	T(e, t), S(e, ae), t[1] = e[0] >>> 1, T(e, t), S(e, oe), t[1] = e[0] >>> 1, T(e, t);
}
var E = [2277735313, 289559509], D = [1291169091, 658871167], O = [0, 5], ce = [0, 1390208809], le = [0, 944331445];
function ue(e, t) {
	let n = ie(e);
	t ||= 0;
	let r = [0, n.length], i = r[1] % 16, a = r[1] - i, o = [0, t], s = [0, t], c = [0, 0], l = [0, 0], u;
	for (u = 0; u < a; u += 16) c[0] = n[u + 4] | n[u + 5] << 8 | n[u + 6] << 16 | n[u + 7] << 24, c[1] = n[u] | n[u + 1] << 8 | n[u + 2] << 16 | n[u + 3] << 24, l[0] = n[u + 12] | n[u + 13] << 8 | n[u + 14] << 16 | n[u + 15] << 24, l[1] = n[u + 8] | n[u + 9] << 8 | n[u + 10] << 16 | n[u + 11] << 24, S(c, E), C(c, 31), S(c, D), T(o, c), C(o, 27), x(o, s), S(o, O), x(o, ce), S(l, D), C(l, 33), S(l, E), T(s, l), C(s, 31), x(s, o), S(s, O), x(s, le);
	c[0] = 0, c[1] = 0, l[0] = 0, l[1] = 0;
	let d = [0, 0];
	switch (i) {
		case 15: d[1] = n[u + 14], w(d, 48), T(l, d);
		case 14: d[1] = n[u + 13], w(d, 40), T(l, d);
		case 13: d[1] = n[u + 12], w(d, 32), T(l, d);
		case 12: d[1] = n[u + 11], w(d, 24), T(l, d);
		case 11: d[1] = n[u + 10], w(d, 16), T(l, d);
		case 10: d[1] = n[u + 9], w(d, 8), T(l, d);
		case 9: d[1] = n[u + 8], T(l, d), S(l, D), C(l, 33), S(l, E), T(s, l);
		case 8: d[1] = n[u + 7], w(d, 56), T(c, d);
		case 7: d[1] = n[u + 6], w(d, 48), T(c, d);
		case 6: d[1] = n[u + 5], w(d, 40), T(c, d);
		case 5: d[1] = n[u + 4], w(d, 32), T(c, d);
		case 4: d[1] = n[u + 3], w(d, 24), T(c, d);
		case 3: d[1] = n[u + 2], w(d, 16), T(c, d);
		case 2: d[1] = n[u + 1], w(d, 8), T(c, d);
		case 1: d[1] = n[u], T(c, d), S(c, E), C(c, 31), S(c, D), T(o, c);
	}
	return T(o, r), T(s, r), x(o, s), x(s, o), se(o), se(s), x(o, s), x(s, o), ("00000000" + (o[0] >>> 0).toString(16)).slice(-8) + ("00000000" + (o[1] >>> 0).toString(16)).slice(-8) + ("00000000" + (s[0] >>> 0).toString(16)).slice(-8) + ("00000000" + (s[1] >>> 0).toString(16)).slice(-8);
}
function de(e) {
	return {
		name: e.name,
...
function he(e, t, n, r) {
	let i = Object.keys(e).filter((e) => te(n, e)), a = g(h(i, (n) => me(e[n], t), r));
	return async function() {
		let e = await h(await a, (e) => g(e()), r), t = await Promise.all(e), n = {};
...
function M() {
	let e = window;
	return fe(e.print) && String(e.browser) === "[object WebPageNamespace]";
}
function ve() {
	let e = window;
	return b([
...
function P() {
	let e = k(), t = ve(), n = window, r = navigator, i = "connection";
	return e ? b([
		!("SharedWorker" in n),
...
function De() {
	...
	let [a, o] = Ae(n), s = g(a.then((e) => je(e.getChannelData(0).subarray(4500)), (e) => {
...
function Ae(e) {
	...
	return [new Promise((n, r) => {
		...
		let s = () => {
			setTimeout(() => r(Me("timeout")), Math.min(500, o + 5e3 - Date.now()));
		}, c = () => {
			try {
				let t = e.startRendering();
				switch (p(t) && g(t), e.state) {
					case "running":
						o = Date.now(), i && s();
						break;
					case "suspended":
						document.hidden || a++, i && a >= 3 ? r(Me("suspended")) : setTimeout(c, 500);
						break;
				}
			} catch (e) {
				r(e);
...
async function Ne(e, t, n = 50) {
	var r;
	let i = document;
	for (; !i.body;) await u(n);
	let a = i.createElement("iframe");
	try {
		for (await new Promise((e, n) => {
			...
			let { style: s } = a;
			s.setProperty("display", "block", "important"), s.position = "absolute", s.top = "0", s.left = "0", s.visibility = "hidden", t && "srcdoc" in a ? a.srcdoc = t : a.src = "about:blank", i.body.appendChild(a);
			let c = () => {
				r || (a.contentWindow?.document?.readyState === "complete" ? o() : setTimeout(c, 10));
			};
			c();
		}); !a.contentWindow?.document?.body;) await u(n);
		return await e(a, a.contentWindow);
	} finally {
		(r = a.parentNode) == null || r.removeChild(a);
	}
}
function Pe(e) {
	let [t, n] = re(e), r = document.createElement(t ?? "div");
	for (let e of Object.keys(n)) {
		let t = n[e].join(" ");
		e === "style" ? Fe(r.style, t) : r.setAttribute(e, t);
	}
	return r;
}
function Fe(e, t) {
...
var Le = "mmMwWLliI0O&1", Re = "48px", F = [
	"monospace",
	"sans-serif",
	"serif"
], I = /* @__PURE__ */ "sans-serif-thin.ARNO PRO.Agency FB.Arabic Typesetting.Arial Unicode MS.AvantGarde Bk BT.BankGothic Md BT.Batang.Bitstream Vera Sans Mono.Calibri.Century.Century Gothic.Clarendon.EUROSTILE.Franklin Gothic.Futura Bk BT.Futura Md BT.GOTHAM.Gill Sans.HELV.Haettenschweiler.Helvetica Neue.Humanst521 BT.Leelawadee.Letter Gothic.Levenim MT.Lucida Bright.Lucida Sans.Menlo.MS Mincho.MS Outlook.MS Reference Specialty.MS UI Gothic.MT Extra.MYRIAD PRO.Marlett.Meiryo UI.Microsoft Uighur.Minion Pro.Monotype Corsiva.PMingLiU.Pristina.SCRIPTINA.Segoe UI Light.Serifa.SimHei.Small Fonts.Staccato222 BT.TRAJAN PRO.Univers CE 55 Medium.Vrinda.ZWAdobeF".split(".");
function ze() {
	return Ne(async (e, { document: t }) => {
		let n = t.body;
		n.style.fontSize = Re;
		...
		let i = {}, a = {}, o = (e) => {
			...
			return i.position = "absolute", i.top = "0", i.left = "0", i.fontFamily = e, n.textContent = Le, r.appendChild(n), n;
		}, s = (e, t) => o(`'${e}',${t}`), c = () => F.map(o), l = () => {
			let e = {};
			for (let t of I) e[t] = F.map((e) => s(t, e));
			return e;
		}, u = (e) => F.some((t, n) => e[n].offsetWidth !== i[t] || e[n].offsetHeight !== a[t]), d = c(), f = l();
		n.appendChild(r);
		for (let e = 0; e < F.length; e++) i[F[e]] = d[e].offsetWidth, a[F[e]] = d[e].offsetHeight;
		return I.filter((e) => u(f[e]));
	});
}
function Be() {
...
function He(e) {
	let t = !1, n, r, [i, a] = Ue();
	return We(i, a) ? (t = Ge(a), e ? n = r = "skipped" : [n, r] = Ke(i, a)) : n = r = "unsupported", {
		winding: t,
		geometry: n,
		text: r
	};
...
function Ke(e, t) {
	qe(e, t);
	let n = L(e);
	return n === L(e) ? (Je(e, t), [L(e), n]) : ["unstable", "unstable"];
}
function qe(e, t) {
...
function Xe() {
	...
	e.maxTouchPoints === void 0 ? e.msMaxTouchPoints !== void 0 && (t = e.msMaxTouchPoints) : t = _(e.maxTouchPoints);
	try {
...
function Qe() {
	...
	if (n !== void 0 && t.push([n]), Array.isArray(e.languages)) k() && ye() || t.push(e.languages);
...
function et() {
	return y(v(navigator.deviceMemory), void 0);
}
function tt() {
	if (!(A() && N() && M())) return nt();
}
function nt() {
	let e = screen, t = (e) => y(_(e), null), n = [t(e.width), t(e.height)];
	return n.sort().reverse(), n;
}
var rt = 2500, it = 10, R, z;
function at() {
	if (z !== void 0) return;
	let e = () => {
		let t = B();
		V(t) ? z = setTimeout(e, rt) : (R = t, z = void 0);
	};
	e();
}
function ot() {
...
function st() {
	if (A() && N() && M()) return () => Promise.resolve(void 0);
	let e = ot();
	return async () => {
		let t = await e(), n = (e) => e === null ? null : ne(e, it);
		return [
			n(t[0]),
			n(t[1]),
			n(t[2]),
...
function B() {
	...
	return [
		y(v(e.availTop), null),
		y(v(e.width) - v(e.availWidth) - y(v(e.availLeft), 0), null),
		y(v(e.height) - v(e.availHeight) - y(v(e.availTop), 0), null),
		y(v(e.availLeft), null)
	];
}
function V(e) {
...
function ct() {
	return y(_(navigator.hardwareConcurrency), void 0);
}
function lt() {
...
function ut() {
	...
	return Math.max(v(new Date(e, 0, 1).getTimezoneOffset()), v(new Date(e, 6, 1).getTimezoneOffset()));
}
function dt() {
	try {
...
function gt() {
	...
	return e === "MacIntel" && A() && !j() ? Se() ? "iPad" : "iPhone" : e;
}
function _t() {
...
async function xt({ debug: e } = {}) {
	if (!St()) return;
	let t = bt(), n = Object.keys(t), r = await Ct([].concat(...n.map((e) => t[e])));
	e && wt(t, r);
	let i = n.filter((e) => {
		let n = t[e];
		return b(n.map((e) => r[e])) > n.length * .6;
	});
	return i.sort(), i;
}
function St() {
...
async function Ct(e) {
	var t;
	let n = document, r = n.createElement("div"), i = Array(e.length), a = {};
	H(r);
	for (let t = 0; t < e.length; ++t) {
		let a = Pe(e[t]);
		a.tagName === "DIALOG" && a.show();
		let o = n.createElement("div");
		H(o), o.appendChild(a), r.appendChild(o), i[t] = a;
	}
	for (; !n.body;) await u(50);
...
function kt() {
	if (matchMedia("(min-monochrome: 0)").matches) {
		for (let e = 0; e <= Ot; ++e) if (matchMedia(`(max-monochrome: ${e})`).matches) return e;
...
function jt() {
	if (K("reduce")) return !0;
	if (K("no-preference")) return !1;
}
function K(e) {
	return matchMedia(`(prefers-reduced-motion: ${e})`).matches;
}
function Mt() {
	if (q("reduce")) return !0;
	if (q("no-preference")) return !1;
}
function q(e) {
...
var Y = Math, X = () => 0;
function Pt() {
	let e = Y.acos || X, t = Y.acosh || X, n = Y.asin || X, r = Y.asinh || X, i = Y.atanh || X, a = Y.atan || X, o = Y.sin || X, s = Y.sinh || X, c = Y.cos || X, l = Y.cosh || X, u = Y.tan || X, d = Y.tanh || X, f = Y.exp || X, p = Y.expm1 || X, m = Y.log1p || X;
	return {
		acos: e(.12312423423423424),
		acosh: t(1e308),
		acoshPf: ((e) => Y.log(e + Y.sqrt(e * e - 1)))(1e154),
		asin: n(.12312423423423424),
		asinh: r(1),
		asinhPf: ((e) => Y.log(e + Y.sqrt(e * e + 1)))(1),
		atanh: i(.5),
		atanhPf: ((e) => Y.log((1 + e) / (1 - e)) / 2)(.5),
		atan: a(.5),
		sin: o(-1e300),
		sinh: s(1),
		sinhPf: ((e) => Y.exp(e) - 1 / Y.exp(e) / 2)(1),
		cos: c(10.000000000123),
		cosh: l(1),
		coshPf: ((e) => (Y.exp(e) + 1 / Y.exp(e)) / 2)(1),
		tan: u(-1e300),
		tanh: d(1),
		tanhPf: ((e) => (Y.exp(2 * e) - 1) / (Y.exp(2 * e) + 1))(1),
		exp: f(1),
		expm1: p(1),
		expm1Pf: ((e) => Y.exp(e) - 1)(1),
		log1p: m(10),
		log1pPf: ((e) => Y.log(1 + e))(10),
		powPI: ((e) => Y.pow(Y.PI, e))(-100)
	};
}
var Ft = "mmMwWLliI0fiflO&1", Z = {
	default: [],
...
function It() {
	return Lt((e, t) => {
		let n = {}, r = {};
		for (let r of Object.keys(Z)) {
			let [i = {}, a = Ft] = Z[r], o = e.createElement("span");
		...
		for (let e of Object.keys(Z)) r[e] = n[e].getBoundingClientRect().width;
		return r;
	});
}
function Lt(e, t = 4e3) {
	return Ne((n, r) => {
		...
		o.width = `${t}px`, o.webkitTextSizeAdjust = o.textSizeAdjust = "none", k() ? a.style.zoom = `${1 / r.devicePixelRatio}` : A() && (a.style.zoom = "reset");
...
var Wt = -1, Gt = -2, Kt = /* @__PURE__ */ new Set([
	10752,
	2849,
	2884,
	2885,
	...
	36795,
	38449
]), Jt = ["FRAGMENT_SHADER", "VERTEX_SHADER"], Yt = [
	"LOW_FLOAT",
	"MEDIUM_FLOAT",
	"HIGH_FLOAT",
	"LOW_INT",
	"MEDIUM_INT",
	"HIGH_INT"
], Xt = "WEBGL_debug_renderer_info", Zt = "WEBGL_polygon_mode";
function Qt({ cache: e }) {
	let t = en(e);
	if (!t) return Wt;
	if (!Q(t)) return Gt;
	let n = an() ? null : t.getExtension(Xt);
	return {
...
function $t({ cache: e }) {
	...
	for (let e of l) {
		let n = t[e];
		o.push(`${e}=${n}${Kt.has(n) ? `=${t.getParameter(n)}` : ""}`);
	}
	if (n) for (let e of n) {
		if (e === Xt && an() || e === Zt && on()) continue;
		...
		for (let e of nn(n)) {
			let r = n[e];
			s.push(`${e}=${r}${qt.has(r) ? `=${t.getParameter(r)}` : ""}`);
		}
	}
	for (let e of Jt) for (let n of Yt) {
		let r = tn(t, e, n);
		c.push(`${e}.${n}=${r.join(",")}`);
	}
...
var ln = {
	...
	contrast: At,
	reducedMotion: jt,
	reducedTransparency: Mt,
	hdr: Nt,
	math: Pt,
	pdfViewerEnabled: Rt,
	architecture: zt,
	applePay: Bt,
	privateClickMeasurement: Ut,
	audioBaseLatency: sn,
	dateTimeLocale: cn,
	webGlBasics: Qt,
	webGlExtensions: $t
...
function fn(e) {
	...
	return {
		score: t,
		comment: dn.replace(/\$/g, `${n}`)
	};
}
function pn(e) {
	if (P()) return .4;
	if (A()) return j() && !(N() && M()) ? .5 : .3;
	let t = "value" in e.platform ? e.platform.value : "";
	return /^Win/.test(t) ? .6 : /^Mac/.test(t) ? .5 : .7;
}
function mn(e) {
	return ne(.99 + .01 * e, 1e-4);
}
function hn(e) {
	let t = "";
...
function gn(e) {
	return JSON.stringify(e, (e, t) => t instanceof Error ? de(t) : t, 2);
}
function _n(e) {
	return ue(hn(e));
}
function vn(e) {
	...
	return {
		get visitorId() {
			return t === void 0 && (t = _n(this.components)), t;
		},
		set visitorId(e) {
			t = e;
		},
...
function bn(e, t) {
	let n = Date.now();
	return { async get(r) {
		let i = Date.now(), a = await e(), o = vn(a);
...
function xn() {
	if (!(window.__fpjs_d_m || Math.random() >= .001)) try {
		...
		e.open("get", `https://m1.openfpcdn.io/fingerprintjs/v${l}/npm-monitoring`, !0), e.send();
	} catch (e) {
		console.error(e);
	}
}
async function Sn(e = {}) {
	(e.monitoring ?? !0) && xn();
	let { delayFallback: t, debug: n } = e;
	return await yn(t), bn(un({
		cache: {},
		debug: n
	}), n);
}
...
async function Dn(e, t) {
	let n = window.location.href, { language: r } = navigator;
	return {
		errorType: e,
		url: n,
		language: r,
		allLanguages: [...navigator.languages],
		browserInfo: wn.default.parse(navigator.userAgent),
		deviceDimensions: `${window.innerWidth}x${window.innerHeight}`,
		browserId: await On(),
		localTime: (/* @__PURE__ */ new Date()).toString(),
		userPreferences: {
...
async function On() {
	return (await (await Cn.load()).get()).visitorId;
}
//#endregion
//#region src/index.ts
function kn(e) {
...
async function An(e, t) {
	...
	let r = await Dn(e, t);
	if (n.debug) {
		console.log(`%c logger-browser %c ${r.errorType} `, "background: #db3534; color: white;", "background: lightgrey; color: #333;", "Endpoint called with parameters:", r);
		return;
	}
	if (!n.logEndpoint) return console.error("Logger endpoint not configured");
	await fetch(n.logEndpoint, {
		method: "POST",
...
//#endregion
export { kn as configure, On as getBrowserId, An as logError };
